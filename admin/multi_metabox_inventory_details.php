    ...
    */
    ...
    class Metabox_List_Table extends WP_List_Table {
        ...
        function getAllRecords(){
            ...
            $metaboxList = $wpdb->get_results("SELECT * FROM ".$table_multi_meta_table ." ORDER BY metabox_field_created_date ASC");            
            $i=0;
            foreach ( $metaboxList as $metabox ) {
                ...
                $metaboxArray[$i]['metabox_field_title']        = $metabox->metabox_title;
                $metaboxArray[$i]['metabox_field_status']       = $status;
                $metaboxArray[$i]['metabox_field_post_type']    = $metabox->metabox_type;
        ...
        function column_title($item){
            ...
            $actions = array(
            'edit'      => sprintf(
                 '<a href="?page=%s&action=%s&id=%s">Edit</a>','multi_metabox_listing','edit',$item['id']
                ),
            'delete'    => sprintf(
                '<a  href="?page=%s&action=%s&id=%s"  class="deleteitem">Delete</a>','multi_metabox_listing','delete',$item['id']
                ),
            );
            ?>
        ...
        function get_columns(){
            ...
            'metabox_field_created_date'  => 'Created Date',
            );
            return $columns;
        }
...
<div class="wrap">
    ...
    <h2>Multiple Metabox Listing <a class="add-new-h2" href="?page=multi_metabox_listing&action=add">Add New</a></h2>
    <?php echo @$message;?>
